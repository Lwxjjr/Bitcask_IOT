...
## 🔄 Hint File 架构设计思路
  ...
  把复杂的混合问题拆成两个简单的线性问题：
   ...
   1. 写数据文件：纯追加写入，不考虑任何索引问题
   2. 重建 Hint File：线性扫描数据文件，提取 Key-Value 位置信息
 ...
 ### 📁 文件结构
   ...
   目录结构：
   ├── segment-001.vlog     # 数据文件（纯追加，只写）
   ├── segment-001.hint     # 索引文件（启动时重建）
   ├── segment-002.vlog
   ├── segment-002.hint
   └── ...
  ...
  要点：
   - 纯追加写入，不需要维护任何内部索引
   - 不需要轮转时写入 Footer 或 Index Block
   - 崩溃恢复时，只需要扫描到最后一个完整的 Block
  ...
  要点：
   - 启动时线性扫描 .vlog 文件生成
   - 也可以在写入时同步追加（权衡：启动速度 vs 写入速度）
   - 内存中只需要维护当前 Active Segment 的 Hint
...
### 🔄 启动恢复流程（简化版）
   ...
   启动流程
     ...
     │   │   └─ 直接加载 Hint 到内存
     │   │
     │   └─ 【路径2】无 Hint 或 Hint 无效
     │       └─ 线性扫描 .vlog 文件
     │           ├─ 读取每个 Block
     │           ├─ 提取 SensorID + Offset + Size
     │           └─ 重建 Hint
     │
     └─ 恢复完成

  关键简化：
   - ✅ 不需要处理 Footer 的 Magic 验证
   - ✅ 不需要处理 Index Block 的解析
   - ✅ 崩溃恢复逻辑：找到最后一个完整的 Block 即可
...
### 🔄 轮转流程（简化版）
   ...
   Segment 轮转
     ...
     │   └─ Close 文件句柄
     │
     ├─ 将当前 Segment 移入 olderSegments
     │
     ├─ 创建新的 Active Segment (nextID)
     │   └─ NewSegment(path, nextID)
     │
     └─ 继续接收写入
...
### 📊 对比：原架构 vs Hint File 架构


  ┌───────────┬───────────────────────────────────────┬───────────────────────────────┐
  │ 维度      │ 原架构（Footer + Index Block）        │ Hint File 架构                │
  ├───────────┼───────────────────────────────────────┼───────────────────────────────┤
  │ **写入复... │ 需要维护内存索引 + 写入时追加 Meta    │ 纯追加，不需要维护内部索引    │
  │ **轮转复... │ 需要写入 Footer（Magic + CRC + Ind... │ 简单关闭旧文件，创建新文件    │
  │ 崩溃恢复  │ 需要验证 Footer，解析 Index Block     │ 线性扫描找到最后一个完整 B... │
  │ 启动加载  │ 需要加载所有 Segment 的 Index Block   │ 可选择加载 Hint 或扫描重建    │
  │ 内存占用  │ Active + Older 都需要维护内存索引     │ 只需维护 Active 的 Hint       │
  │ **代码复... │ 高（需要处理 Footer 编码）            │ 低（线性扫描即可）            │
  └───────────┴───────────────────────────────────────┴───────────────────────────────┘
